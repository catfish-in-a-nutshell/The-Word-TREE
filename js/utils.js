...
function updateMilestones(layer) {
	...
	for (id in layers[layer].milestones) {
		if (!(hasMilestone(layer, id)) && layers[layer].milestones[id].done()) {
			...
			if (tmp[layer].milestonePopups || tmp[layer].milestonePopups === undefined) doPopup("milestone", tmp[layer].milestones[id].requirementDescription, "Milestone Gotten!", 3, tmp[layer].color, tmp[layer].textcolor);
...
function updateAchievements(layer) {
	...
	for (id in layers[layer].achievements) {
		if (isPlainObject(layers[layer].achievements[id]) && !(hasAchievement(layer, id)) && layers[layer].achievements[id].done()) {
			...
			if (tmp[layer].achievementPopups || tmp[layer].achievementPopups === undefined) doPopup("achievement", tmp[layer].achievements[id].name, "Achievement Gotten!", 3, tmp[layer].color, tmp[layer].textcolor);
		}
	}
}
...
function toValue(value, oldValue, isText) {
...
function doPopup(type = "none", text = "This is a test popup.", title = "", timer = 3, color = "", textcolor = "") {
	switch (type) {
		case "achievement":
